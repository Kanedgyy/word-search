...
export default function LoginPage() {
  ...
  const [oauthEnabled, setOauthEnabled] = useState({ github: false, google: false });

  // Проверяем, включены ли OAuth провайдеры
  useEffect(() => {
    // Проверяем по наличие кнопок на странице (фолбэк)
    // В продакшне Better Auth вернёт 404 если провайдер не включён
    setOauthEnabled({ 
  ...
  // Фолбэк: если OAuth не включён, показываем ошибку
  const handleOAuthFallback = (provider: string) => {
    setError(`${provider} OAuth не настроен. Пожалуйста, используйте email/пароль.`);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900 p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 w-full max-w-md border border-white/20">
        ...
        <div className="space-y-3">
          ...
          {!oauthEnabled.github && !oauthEnabled.google && (
            <div className="text-center text-purple-200 text-sm py-4">
              OAuth провайдеры не настроены. Пожалуйста, используйте email/пароль.
            </div>
          )}
        </div>
